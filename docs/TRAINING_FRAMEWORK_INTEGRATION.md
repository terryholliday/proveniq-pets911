...
## 5. Existing Code Alignment

The following existing modules already partially implement the research framework:

| Existing File | Alignment Status | Notes |
|---------------|------------------|-------|
| `volunteer-governance.ts` | ✅ Badge system exists | Add research-aligned badge definitions |
| `capabilities.ts` | ✅ Skills defined | Add certification track requirements |
| `status.ts` | ✅ Compassion fatigue tracking | Add 2-Hour Hard Stop logic |
| `dispatch.ts` | ⚠️ Priority exists | Add triage code mapping |
| `training/types.ts` | ✅ Certification structure | Link to new module registry |
...
## 7. Content Development Notes for Claude
...
## 8. Database Schema Additions (Future)
