...
export default function ModeratorConsoleDashboard() {
  ...
  const { queueAction } = useOfflineQueue(null);
  ...
  const handleLockCase = async (caseId: string, caseType: 'missing' | 'found') => {
    ...
    setCases((prev) => prev.map((c) => (c.id === caseId ? { ...c, status: 'LOCKED' as const } : c)));
  };
  ...
  const handleUnlockCase = async (caseId: string, caseType: 'missing' | 'found') => {
    ...
    setActionBanner({ variant: 'success', message: 'Unlock queued for sync.' });
  };
  ...
  const handleCloseCase = async (caseId: string, caseType: 'missing' | 'found') => {
    ...
    try {
      ...
      setCases((prev) => prev.map((c) => (c.id === caseId ? { ...c, status: reason as CaseStatus } : c)));
    ...
    <div className="min-h-screen bg-gray-100">
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-xl font-bold">Moderator Console</h1>
      ...
      <main className="max-w-6xl mx-auto p-4">
                ...
                <div className="mt-5 space-y-5">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700">
                      Type <span className="font-mono">ESCALATE</span> to confirm
                    </label>
                    <input
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          ...
          />
          <StatCard icon={<GitMerge className="h-5 w-5 text-green-600" />} label="Pending Matches" value={matches.length} />
          <StatCard
        ...
        <div className="flex gap-2 mb-4">
          <Button variant={activeTab === 'triage' ? 'default' : 'outline'} onClick={() => setActiveTab('triage')}>
          ...
          <Button variant={activeTab === 'sightings' ? 'default' : 'outline'} onClick={() => setActiveTab('sightings')}>
        ...
        {activeTab === 'triage' ? (
          ...
          <div className="space-y-4">
            {matches.length === 0 ? (
              <Card>
                <CardContent className="py-8 text-center text-gray-500">No pending match suggestions</CardContent>
              </Card>
            ) : (
              <>
                <Alert variant="info">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    <strong>Reminder:</strong> Match suggestions are AI-generated and require human verification before confirming. Never share match details with owners until confirmed.
                  </AlertDescription>
                </Alert>
                {matches.map((match) => (
                  <MatchReviewCard key={match.match_id} match={match} onConfirm={handleConfirmMatch} onReject={handleRejectMatch} />
                ))}
              </>
            )}
          </div>
