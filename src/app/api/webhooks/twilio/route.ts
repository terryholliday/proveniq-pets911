...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!volunteer) {
      return new NextResponse(
        '<?xml version="1.0" encoding="UTF-8"?><Response><Message>Volunteer profile not found. Please register at Mayday.org/helpers/join</Message></Response>',
        { 
          status: 200,
          headers: { 'Content-Type': 'text/xml' }
        }
    ...
    if (action === 'ACCEPTED') {
      ...
      return new NextResponse(
        '<?xml version="1.0" encoding="UTF-8"?><Response><Message>Thank you! You\'ve accepted the dispatch. The finder will be notified. Check your dashboard for details: Mayday.org/helpers/dashboard</Message></Response>',
        { 
          status: 200,
          headers: { 'Content-Type': 'text/xml' }
        }
